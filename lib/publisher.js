...
class Publisher {
    constructor(options) {
        ...
        this._topicLocatlizer = new internal_1.TopicLocatlizer(options);
    ...
    async _getConnection(topic) {
        ...
        if (typeof connections === "undefined") {
            connections = [];
            for (let i = 0; i < this._options.publisher.connection_pool_slot_size; i++) {
                const connection = new maxwell_utils_1.Connection(endpoint, this._options.connection);
